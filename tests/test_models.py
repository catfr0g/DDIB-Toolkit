...
def test_resnet_bottleneck_none_vs_int():
	...
	model_with_bottleneck = ResNetWithBottleneck(
		arch='resnet18', num_classes=10, bottleneck_width=128
	)

	model_without_bottleneck = ResNetWithBottleneck(
		arch='resnet18', num_classes=10, bottleneck_width=None
	)
	...
	assert out1.shape == out2.shape == (4, 10), (
		f'Output shapes should match: {out1.shape}, {out2.shape}'
	)
...
def test_vgg_bottleneck_none_vs_int():
	...
	# Create models with and without bottleneck
	model_with_bottleneck = VGGWithBottleneck(arch='vgg11', num_classes=10, bottleneck_width=128)

	model_without_bottleneck = VGGWithBottleneck(
		arch='vgg11', num_classes=10, bottleneck_width=None
	)
