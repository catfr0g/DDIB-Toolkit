"""
Unified Grid Search Analysis (ResNet + VGG).

Combines experiment_result_analisys.py + numerical_analisys.py into a single script.
Analyzes ResNet and VGG grid search results with baseline comparison.

Usage:
    python notebooks/analyze_grid_search.py
"""
...
df_main = load_grid_search_results(str(RESULTS_PATH))
df_baseline_raw = load_grid_search_results(str(BASELINE_PATH))
non_efficientnet_models = ['resnet18', 'vgg11']
df_baseline = df_baseline_raw[df_baseline_raw['model_arch'].isin(non_efficientnet_models)]
print(f'Loaded {len(df_main)} main, {len(df_baseline)} baseline (excluding EfficientNet)')
df = pd.concat([df_main, df_baseline], ignore_index=True)
print(f'Total: {len(df)} experiments')

df_baseline_only = df_baseline[df_baseline['beta'] == 0]
...
ax.set_title('Test Accuracy: Compression vs Capacity')
ax.set_xscale('log')
ax.set_yscale('log')
ax.grid(True, alpha=0.3, which='both')
plt.colorbar(s, label='Test Accuracy (%)')
plt.tight_layout()
fig.savefig(OUTPUT_DIR / 'scatter_acc_compression_capacity.png', dpi=150)
print('Saved: scatter_acc_compression_capacity.png')
plt.close('all')

# Scatter: compression vs accuracy (color=beta, size=width)
fig, ax = plt.subplots(figsize=(10, 6))
...
ax.grid(True, alpha=0.3, which='both')
cb = plt.colorbar(s)
cb.set_label('Beta (log scale)')
lh = [
	plt.scatter(
		[], [], s=np.log(x) * 20, color='gray', alpha=0.5, edgecolors='black', label=f'Width:{x}'
	)
...
report = f"""Grid Search Results Analysis
{'=' * 60}
Total experiments: {len(df)}
