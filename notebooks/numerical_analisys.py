...
# %%
# Load data (exclude EfficientNet from baseline - separate experiment)
...
df_main = load_grid_search_results(RESULTS_PATH)
df_baseline_raw = load_grid_search_results(BASELINE_PATH)
# Filter baseline to only include non-EfficientNet models (resnet18, vgg11)
...
print(f'Loaded {len(df_main)} main experiments')
print(f'Loaded {len(df_baseline)} baseline experiments (excluding EfficientNet)')
...
# Get baseline stats from full data (includes beta=0)
...
# Create a significance matrix (True/False)
...
# Display as heatmap with annotations
...
rf_importance = pd.DataFrame(
	{'Feature': X_full.columns, 'Importance': rf_results['feature_importances'].values()}
...
gb_importance = pd.DataFrame(
	{'Feature': X_full.columns, 'Importance': gb_results['feature_importances'].values()}
).sort_values('Importance', ascending=True)
